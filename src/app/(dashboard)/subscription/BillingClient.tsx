  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      {["PAST_DUE", "HALTED"].includes(subscriptionStatus) && (
        <div className="bg-red-50 border border-red-200 rounded-2xl p-6 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Button
            ...
            className="bg-red-600 hover:bg-red-700 text-white font-bold text-xs px-5 py-2.5 rounded-xl shrink-0 shadow-xs"
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 sm:p-8">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 border-b border-slate-100">
          <div>
            <h3 className="text-xl font-bold text-slate-900">Available Subscription Plans</h3>
            <p className="text-xs text-slate-500 mt-1">Upgrade or modify your practice plan with 1-click self-managed checkout.</p>
          </div>
        ...
        {/* Pricing Cards Grid: Maximum 3 Columns per Row */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-8">
          {availablePackages.map((pkg) => {
            ...
            return (
              ...
              >
                {/* Clean Top Header Banners */}
                {isCurrent && (
                  <div className="bg-blue-600 text-white text-[11px] font-bold uppercase tracking-wider text-center py-2 flex items-center justify-center gap-1.5">
                    <ShieldCheck className="w-4 h-4" /> Current Active Plan
                  </div>
                )}

                {isPopular && !isCurrent && (
                  <div className="bg-amber-500 text-white text-[11px] font-bold uppercase tracking-wider text-center py-2 flex items-center justify-center gap-1.5">
                ...
                {isAiReceptionist && !isCurrent && !isPopular && (
                  <div className="bg-blue-600 text-white text-[11px] font-bold uppercase tracking-wider text-center py-2 flex items-center justify-center gap-1.5">
                    <Bot className="w-4 h-4" /> 24/7 AI Receptionist & Booking
                  </div>
                )}
                ...
                <div className="p-7 space-y-5">
                  ...
                  {/* Starter Plan Quarterly Commitment Notice */}
                  {isStarter && (
                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-[11px] text-slate-800 font-medium leading-tight space-y-0.5">
                      <div className="font-bold flex items-center gap-1.5 text-slate-900">
                        <CreditCard className="w-3.5 h-3.5 text-blue-600 shrink-0" />
                        <span>Quarterly Commitment Plan</span>
                      </div>
                      <p className="text-[10px] text-slate-600 mt-0.5">Requires a 3-Month commitment for new clinics.</p>
                    </div>
                  )}
                  ...
                  <div className="pt-1">
                    {isFree ? (
                      <div className="flex items-baseline gap-1">
                        <span className="text-4xl font-extrabold text-slate-900">{currency}0</span>
                        <span className="text-xs font-semibold text-slate-500">/mo</span>
                      </div>
                    ) : discountPercent > 0 ? (
                      <div className="space-y-1">
                        <div className="flex items-baseline gap-2.5 flex-wrap">
                          <span className="text-sm font-bold line-through text-red-500 decoration-red-500">{currency}{baseMonthly}</span>
                          <span className="text-4xl font-extrabold text-slate-900">{currency}{effectiveMonthly}</span>
                          <span className="text-xs font-semibold text-slate-500">/mo effective</span>
                        </div>
                        <div className="flex items-center gap-2 text-xs flex-wrap pt-1">
                          <span className="text-slate-500 font-medium">Billed {effectivePeriod}:</span>
                          <span className="line-through text-red-500 font-bold text-xs">{currency}{realTotal}</span>
                          <span className="font-bold text-slate-900">{currency}{offeredTotal}</span>
                          <span className="bg-emerald-100 border border-emerald-200 text-emerald-800 text-[10px] font-bold px-2 py-0.5 rounded-md">
                      ...
                      <div className="flex items-baseline gap-1">
                        <span className="text-4xl font-extrabold text-slate-900">{currency}{baseMonthly}</span>
                        <span className="text-xs font-semibold text-slate-500">/mo</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="border-t border-slate-100 pt-5 space-y-3.5">
                    <div className="text-[11px] font-bold uppercase tracking-wider text-slate-500">What&apos;s Included:</div>
                    ...
                    <ul className="space-y-2.5 text-xs text-slate-700">
                      ...
                      {pkg.modules && pkg.modules.length > 0 ? (
                        pkg.modules.map((m: any) => {
                          ...
                          return (
                            <li key={mName} className="flex items-start gap-2.5 font-medium text-slate-800">
                        ...
                        (() => {
                          ...
                          if (pName.includes("FREE")) {
                            return (
                              <>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> Standard Patient Records & Calendar</li>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> 1 Staff Seat & Basic Billing</li>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> Up to 50 Patient Capacity</li>
                              </>
                            );
                          }
                          if (pName.includes("STARTER")) {
                            return (
                              <>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> Everything in Free</li>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> Google Business Profile & Local Search</li>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> 3 Staff Seats & 500 Patient Records</li>
                              </>
                            );
                          }
                          if (pName.includes("RECEPTIONIST")) {
                            return (
                              <>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> 24/7 WhatsApp AI Receptionist & Booking</li>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> Multilingual Patient Support (Hindi, English, etc.)</li>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> OPD Calendar & Appointment Reminders</li>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> 3 Staff Seats & 2,000 Patient Records</li>
                              </>
                            );
                          }
                          if (pName.includes("GROWTH")) {
                            return (
                              <>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> Everything in Starter</li>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> WhatsApp CRM & Automated Reminders</li>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> 10 Staff Seats & Unlimited Patients</li>
                                <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> Tracked Local SEO Keywords</li>
                              </>
                            );
                          }
                          return (
                            <>
                              <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> Everything in Growth</li>
                              <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> AI Practice Assistant & Auto Reviews</li>
                              <li className="flex items-center gap-2.5 font-medium text-slate-800"><Check className="h-4 w-4 text-emerald-600 shrink-0" /> Unlimited Staff Seats & Multi-Location</li>
                            </>
                          );
                        })()
                      )}
                      ...
                      {pkg.limits && pkg.limits.length > 0 && (
                        pkg.limits.map((l: any) => {
                          ...
                          return (
                            <li key={l.limitName} className="flex items-center justify-between text-[11px] pt-1.5 border-t border-slate-100 text-slate-600 font-medium">
                              <span>{name}</span>
                              <span className="font-semibold text-slate-900 bg-slate-100 px-2 py-0.5 rounded text-[11px]">{val}</span>
                            </li>
                          );
                        })
                      )}
                ...
                <div className="p-6 border-t border-slate-100 bg-slate-50/50 mt-auto">
                  {isCurrent ? (
                    <Button 
                      variant="outline" 
                      className="w-full text-blue-700 border-blue-200 bg-blue-50 font-semibold text-xs cursor-default h-11" 
                      disabled
                    >
                      ✓ Current Active Plan
                    </Button>
                    ...
                    (() => {
                      ...
                      return (
                        <Button 
                          ...
                          className={`w-full font-semibold text-xs shadow-xs transition-all h-11 ${
                        ...
                        >
                          {loadingPkgId === pkg.id ? (
                            <><RefreshCcw className="h-4 w-4 mr-2 animate-spin" /> Processing...</>
                          ) : (
                            <><ArrowRight className="h-4 w-4 mr-2" /> {actionLabel}</>
                          )}
                        </Button>
                      );
                    })()
