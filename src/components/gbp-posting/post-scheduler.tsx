  ...
  const handleSubmit = async (isScheduled: boolean) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Clear Composer
        setForm({
          title: "", content: "", postType: "STANDARD", scheduledDate: "", imageUrl: "", ctaType: "NONE", ctaLink: "", eventTitle: "", eventStartDate: "", eventEndDate: ""
        })
        fetchHistory()
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-10 items-start">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden flex flex-col h-full ring-1 ring-black/5">
          ...
          <div className="p-6 space-y-6 flex-1">
            ...
            {/* Event Details (Shown when Post Type is EVENT) */}
            {form.postType === "EVENT" && (
              <div className="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100 space-y-3.5 animate-fadeIn">
                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-indigo-600" />
                  <span className="text-xs font-bold text-slate-800">Event Schedule (Google Required)</span>
                </div>
                <div>
                  <Label htmlFor="event-title-input" className="text-xs font-semibold text-slate-700 mb-1 block">
                    Event Title (Optional, defaults to post headline)
                  </Label>
                  <input
                    id="event-title-input"
                    type="text"
                    maxLength={58}
                    placeholder="e.g. Free Liver Health Checkup Camp"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <Label htmlFor="event-start-date" className="text-xs font-semibold text-slate-700 mb-1 block">
                      Event Start Date
                    </Label>
                    <input
                      id="event-start-date"
                  ...
                  <div>
                    <Label htmlFor="event-end-date" className="text-xs font-semibold text-slate-700 mb-1 block">
                      Event End Date
                    </Label>
                    <input
                      id="event-end-date"
                  ...
                  <tr key={post.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          {post.lastError && (
                            <p className="text-[11px] text-rose-600 mt-1 flex items-start gap-1 font-medium bg-rose-50/80 p-1.5 rounded-lg border border-rose-100">
                    ...
                    <td className="px-6 py-4">
                      {post.status === "PUBLISHED" ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-rose-50 text-rose-700 border border-rose-200" title={post.lastError || "Publishing failed"}>
                      ...
                      {(post.status === "DRAFT" || post.status === "SCHEDULED" || post.status === "FAILED") && (
                        ...
                        >
                          {publishingDraftId === post.id ? (
                            ...
                            <><Globe className="h-3.5 w-3.5 mr-1" /> Retry Publish</>
                          ) : post.status === "SCHEDULED" ? (
