...
export async function generateInvoicePDF(invoice: InvoiceWithDetails): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const patientCode = invoice.patient.id ? `PAT-${invoice.patient.id.slice(-6).toUpperCase()}` : 'PAT-N/A';
      ...
      doc.fontSize(8).font('Roboto').fillColor('#64748B');
      const footerText = 'Generated by GYREX· | www.gyrex.in |';
      doc.text(footerText, 50, footerY, { width: 500, align: 'center' });

      // Add clickable link area over "www.gyrex.in"
      try {
        const fullWidth = doc.widthOfString(footerText);
        const prefixWidth = doc.widthOfString('Generated by GYREX· | ');
        const linkWidth = doc.widthOfString('www.gyrex.in');
        const startX = 50 + (500 - fullWidth) / 2 + prefixWidth;
        doc.link(startX, footerY, linkWidth, 10, 'https://www.gyrex.in');
      } catch (linkErr) {
        console.warn('Footer link annotation skipped:', linkErr);
      }
