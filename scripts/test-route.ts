...
export async function GET(request: Request) {
  try {
    ...
    // If no cached insights, the requested month differs, or missing new metric fields, fetch fresh data
    if (!insights || !insights.totalSearches || insights.fetchedMonth !== fetchedMonth || typeof insights.viewsChange === 'undefined') {
      try {
    ...
    const responseData = {
      ...
      traffic: {
        topKeywords: (insights.searchKeywords || [])
